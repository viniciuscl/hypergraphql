...
public class SPARQLServiceConverter {
    ...
    private final boolean isD2RServer;

    public SPARQLServiceConverter(HGQLSchema schema, boolean isD2RServer) {
        this.schema = schema;
    ...
    public String getSelectQuery(JsonNode jsonQuery, Set<String> input, String rootType) {
        ...
        if (root) {
            ...
            } else if (queryFields.get(jsonQuery.get("name").asText()).type().equals(HGQLVocabulary.HGQL_QUERY_GET_BY_ID_FIELD)) {  
            	return getSelectRoot_GET_BY_ID(jsonQuery);
            } else {
                return getSelectRoot_GET_BY_PROPERTY(jsonQuery);
            }
        } else {
    ...
    private String getSelectRoot_GET_BY_ID(JsonNode queryField) {
        ...
        if(this.isD2RServer) {	        
	        ...
	        return selectQuerySTR(valuesSTR + selectTriple + subQuery, graphID);
        }
    }

    private String getSelectRoot_GET_BY_PROPERTY(JsonNode queryField) {
        String targetName = queryField.get("targetName").asText();
        String targetURI = schema.getTypes().get(targetName).getId();
        String graphID = ((SPARQLEndpointService) schema.getQueryFields().get(queryField.get("name").asText()).service()).getGraph();
        String nodeId = queryField.get("nodeId").asText();
        String selectTriple = tripleSTR(varSTR(nodeId), RDF_TYPE_URI, uriSTR(targetURI));
        String propertyURI = queryField.get("args").get("propertyURI").asText();
        String propertyValue = queryField.get("args").get("propertyValue").asText();
        
        if(queryField.get("args").get("propertyValue").asText().contains("http")) {
        	selectTriple += tripleSTR(varSTR(nodeId), uriSTR(propertyURI), uriSTR(propertyValue));
        } else {
        	selectTriple += tripleSTR(varSTR(nodeId), uriSTR(propertyURI), "'"+propertyValue+"'");
        }
                                     
	    JsonNode subfields = queryField.get("fields");
	    String subQuery = getSubQueries(subfields);
	
	    return selectQuerySTR(selectTriple + subQuery, graphID);
    }
